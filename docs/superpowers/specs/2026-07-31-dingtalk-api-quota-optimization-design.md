...
**价格指数组合冗余**：实际展开 340 组合，写入 171 组合。用户确认精简如下：

- 排除小麦品种（48 组合）
- 玉米深加工企业收购价从 67 家保留 12 家代表性企业（每个地区 2-3 家）
- 保留玉米主流粮成交价（38 港口）+ 玉米深加工保留（12）+ 国产大豆贸易商报价（18）= **68 组合**
...
### 3. 价格指数组合精简（`crawler/price_index.py` + `config.py`）

`expand_all_combinations` 增加两级过滤：

1. **排除品种**：`PRICE_INDEX_EXCLUDE_VARIETIES`（逗号分隔，默认空），部署设 `小麦`。
2. **深加工收购点白名单**：`PRICE_INDEX_DEEP_KEEP_AREAS`（逗号分隔，默认空），对价格类型为"深加工企业收购价"的组合，仅保留名单中的企业，其余跳过。部署设 12 家代表性企业：

   中粮榆树、吉林梅花、京粮龙江、寿光金玉米、诸城兴贸、福洋生物、河南汉永、孟州金玉米、宁夏伊品、宝鸡阜丰、蚌埠丰原、骊骅原料

- 已同步的小麦与未保留深加工企业文档**保留不动**，仅停止采集与更新。
...
- 新增配置 `MONTHLY_QUOTA_LIMIT = 4000`（2026-07-31 实测精简后组合 131，较预估 68 多约一倍，熔断从 4800 收紧至 4000，为 gettoken 与 create_document 预留额度空间）。
...
| `PRICE_INDEX_DEEP_KEEP_AREAS` | `""` | 深加工企业收购价保留的收购点白名单，逗号分隔（12 家代表性企业） |
| `DAILY_WRITE_COOLDOWN_HOURS` | `24` | 同文档每日冷却期（小时） |
| `MONTHLY_QUOTA_LIMIT` | `4000` | 月度覆盖写入熔断阈值（预留 gettoken 与 create_document 额度） |

## 预算估算（优化后，每月）

2026-07-31 实测精简后价格指数组合为 **131**（预估 68：设计按地点数算，实际每个收购点含约 2 个等级）。最坏情况预算如下：

| 来源 | 每月调用 |
|---|---|
| 覆盖写入（价格指数 131 组合全变 + 华南/海南 + 新增文章） | ≤4000（熔断硬顶） |
| `gettoken` | ≈360 |
| `create_document` 新增文档 | ≈100–230 |
| **合计（最坏）** | **≈4460–4590 < 5000** ✓ |

熔断阈值 4000 作为硬性兜底：即使价格指数每天全部 131 组合内容变化（≈3980）并叠加其他覆盖写入，覆盖写入也会在 4000 处熔断；再叠加固定的 gettoken（≈360）与新增 create_document（≈230），月度总调用硬性不超过约 4590，仍低于 5000 配额。
...
8. **组合精简**：`PRICE_INDEX_EXCLUDE_VARIETIES`（小麦）与 `PRICE_INDEX_DEEP_KEEP_AREAS`（深加工白名单）均生效，小麦与未保留深加工企业组合被过滤。
...
- **熔断触发**：若某月覆盖写入逼近 4000，部分更新推迟到次日/次月，通过日志告警可见，不会超量计费。
...
- 价格指数精简：排除小麦（48）；深加工企业收购价保留 12 家代表性企业（67→12）；保留玉米港口 38 + 大豆 18，共 68 组合（预估）。2026-07-31 实测每个收购点含约 2 个等级，精简后实际 131 组合。
- 熔断阈值：2026-07-31 收紧为 4000（原设计 4800），为 gettoken 与 create_document 预留额度空间。
- 已有小麦/未保留深加工企业文档：保留不动，停止更新。
